...
try {
  ...
  try {
    ...
    var puppeteerVersion = require(require.resolve('puppeteer/package.json', { paths: Module._nodeModulePaths(process.cwd()) })).version;
    ...
    try {
      ...
      try {
        // In our custom fork we will include the module "puppeteer v17.1.3" and install it here in node_modules
        var puppeteer = require('puppeteer');
        var puppeteerVersion = require('puppeteer/package.json').version;
      } catch (forkError) {
  ...
  // lock to version 17.1.3 that is working with our available binaries installed on stage servers
  if (puppeteerVersion != null && puppeteerVersion != "17.1.3") {
    throw `The only working puppeteer version is 17.1.3 and you are trying to use ${puppeteerVersion}. Please install the right one!`;
  }
} catch (e) {
